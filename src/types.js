      ...
      return Pipe(constructor(val))
        .andThen(
          v =>
            typecheck(v)
              ? v
              : throwError(`Value '${val}' is not of expected type '${name}'`)
        )
...
const typecheckData = typeDef => val =>
  ...
  Object.entries(typeDef).filter(([key, type]) => !type.is(val[key])).length ===
    0;
...
const Data = (name, typeDef) =>
  ...
  isValidDataDefinition(typeDef)
    ? Pipe(normalizeDefinition(typeDef))
        .andThen(def => Type(name, typecheckData(def), constructData(def)))
        .value()
    : throwError(
